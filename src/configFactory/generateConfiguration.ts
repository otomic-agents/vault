...
async function main() {
    ...
    if (evm == false && solana == false) {
        logger.warn('Use "-- --evm" to generate the configuration for the EVM signature server, or use "-- --solana" to generate the configuration for the Solana signature server. If you want to configure both EVM and Solana at the same time, use "-- --evm --solana".')
        return
    }
    ...
    if (evm == true) {
        ...
        while (!fs.existsSync(evmKeystoreFile)) {
            const pathEvmKeystore = await promptTextNoDefault(`Could not find the EVM keystore file at path ${defEvmKeystorePath}, please enter your keystore file path:`)
    ...
    if (solana == true) {
        ...
        while (!fs.existsSync(solanaKeystoreFile)) {
            const pathEvmKeystore = await promptTextNoDefault(`Could not find the Solana keystore file at path ${defSolanaKeystorePath}, please enter your keystore file path:`)
    ...
    // get lpnode ip
    const ip = await promptTextNoDefault(`Please enter the IP address of the device running the Otmoic LPNode program(If the signature server is on the same local network, you should enter its internal IP address):`)
...
const rewriteConfig = async (evm: boolean, solana: boolean, evmKeystoreFile: string, solanaKeystoreFile: string, evmPassword: string, solanaPassword: string, port: string, ip: string) => {
...
const rewriteRemoveEVMConfigDockerCompose = () => new Promise<void>((resolve, reject) => {
...
const rewriteRemoveSolanaConfigDockerCompose = () => new Promise<void>((resolve, reject) => {
    const filePath = './signer/docker-compose.yml';
...
const rewriteEVMConfigNginx = () => new Promise<void>((resolve, reject) => {
...
const rewriteSolanaConfigNginx = () => new Promise<void>((resolve, reject) => {
    // Define the path to the nginx configuration file
    const filePath = './signer/nginx.conf';
...
const rewriteEVMPassword = (password: string) => new Promise<void>((resolve, reject) => {
    const filePath = './signer/docker-compose.yml';
...
const rewriteSolanaPassword = (password: string) => new Promise<void>((resolve, reject) => {
    const filePath = './signer/docker-compose.yml';
...
const rewriteWhiteLists = (ip: string) => new Promise<void>((resolve, reject) => {
